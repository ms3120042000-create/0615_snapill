...
7. 약품명을 이미지에서 읽었다면, 그 약품의 성분(ingredients)·효능(efficacy)·분류(category)·주의사항(precaution)은 반드시 너의 한국 의약품 지식으로 채워줘. 이미지에 없다고 빈 문자열로 두면 안 돼.
8. 절대 완전히 근거 없이 만들지 말고, 약품명이 확인된 경우에는 공식 약품 정보를 기반으로 채워줘. 약품명조차 판독 불가능한 경우만 빈 문자열로 둬.
